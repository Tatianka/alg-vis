...
public class FingerNode2 extends Node {
	
	int width, leftw, rightw;
	FingerNode2 parent = null, leftNeigbour = null, rightNeighbour = null; 
	...
	public FingerNode2(DataStructure D, int key, int x, int y) {
		this.key = new int[9];
		c = new FingerNode2[9];
		this.key[0] = key;
		numKeys = 1;
	...
	public FingerNode2(DataStructure D, int key) {
		this(D, key, 0, 0);
		setState(Node.UP);
		//super(D,key);
	}
	
	public FingerNode2(FingerNode2 v) {
	...
	public FingerNode2 split() {
		...
		p.c[0] = ch1;
		p.c[1] = ch2;
	// what is it??	-from FingerNode2.split
...
//////////////////////////////////////////////////////////////////	
	public void add(int k, FingerNode2 v) {
	...
	public FingerNode2 del(int k) {
		...
		width = _width();
		return new FingerNode2(D, k, x - (numKeys + 1 - 2 * p) * D.radius, y);
	}

	public FingerNode2 delMin() {
		int r = key[0];
		--numKeys;
		...
		width = _width();
		return new FingerNode2(D, r, x - (numKeys - 1) * D.radius, y);
	}

	public FingerNode2 delMinCh() {
		FingerNode2 r = c[0];
		--numChildren;
		for (int i = 0; i < numChildren; ++i) {
			c[i] = c[i + 1];
	...
	public FingerNode2 delMax() {
		FingerNode2 r = new FingerNode2(D, key[--numKeys], x + (numKeys - 1) * D.radius, y);
		width = _width();
		return r;
	}

	public FingerNode2 delMaxCh() {
		FingerNode2 r = c[--numChildren];
		width = _width();
		return r;
	}
	...
	public void insMinCh(FingerNode2 v) {
	...
	public void insMaxCh(FingerNode2 v) {
		c[numChildren++] = v;
		width = _width();
	}
	...
	public void rebox() {
		if (numChildren == 0) {
			leftw = rightw = width / 2 + ((FingerTree2) D).xspan; // numKeys *
			// D.radius +
			// D.xspan;
		} else {
			if (numChildren % 2 == 0) {
	...
	public int _goToX(FingerNode2 v) {
	...
	public void goTo(FingerNode2 v) {
		goTo(_goToX(v), v.toy);
	}

	public void goAbove(FingerNode2 v) {
		goTo(_goToX(v), v.toy - 2 * D.radius + 2);
	}

	public void goBelow(FingerNode2 v) {
		goTo(_goToX(v), v.toy + 2 * D.radius - 2);
	}
